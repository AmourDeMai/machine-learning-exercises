...
int MAXCYCLES=10000; //defines the maximum number of training cycles.
double RMS_ERROR_THRESHOLD=0.01;  //defines the stopping threshold during training.
...
void read_NNdescription(char * filename)
{
  ...
  for (int i=0; i<nb_layers; i++) {
    InFile >> layers[i].size;
  }

  for (int i=0; i<nb_layers; i++) {
    if(i!=outlayer()) layers[i].size+=1; //add an extra neuron for the bias
...
void read_NNweights(char *filename)
{
  ...
  for (int i=1; i<nb_layers; i++) {
    for (int j=0; j<layers[i-1].size; j++) {
      // deals with the bias neuron on each layer but the outlayer()
      int k;
      if(i!=outlayer()) k=1;
      else k=0;
			
      for (; k<(layers[i].size); k++) {
		InFile >> layers[i].w[j][k];
      }
    }
  }
  InFile.close();
...
void read_patterns(char *filename)
{
  ...
  // in the input data file, the first number is nb_pattern
  InFile >> nb_pattern;
  Alloc2D(pattern,double,(unsigned)nb_pattern,(unsigned)((layers[0].size-1)+layers[outlayer()].size));

  int i=0;
  int j=0;
  while(InFile.good() && !InFile.eof() && i<nb_pattern) {
    for (j=0; j<layers[0].size-1; j++) {
      InFile >> pattern[i][j];
    }
    for (int k=0; k<layers[outlayer()].size; k++) {
      InFile >> pattern[i][j+k];
    }
    i++;
  }
  if(i!=nb_pattern) {
...
void feedforward(int pat)
{
  ...
  for(int i=1; i<nb_layers; i++) {
	...
	// iterate every neuron of the layer
    for(; k<layers[i].size; k++) {
      //
      // IMPLEMENTATION REQUIRED
      // reset the activation u of neuron k in layer i
      layers[i].u[k] = 0.0;
...
void backpropagation(int pat) {
  //compute the error and dE_du at the output layer (outlayer())
  for(int i=0; i < layers[outlayer()].size; i++) {
    ////
    // IMPLEMENTATION REQUIRED
    //
    // compute the difference between the network ouput and the desired output (pattern[pat][])
	double diff = layers[outlayer()].o[i] - pattern[pat][i];
	// compute the local contribution to the RMS error (rms_error+=square of the difference computed above)
	rms_error += SQR(diff);
    // compute the partial derivative of the error with respect to the activation at the output layer (layers[outlayer()].dE_du[]) using the derivative of the activation function (deriv_transfer_f(layers[].u[]))
	layers[outlayer()].dE_du[i] = 2 * diff * deriv_transfer_f(layers[outlayer()].u[i]);
    //
    // END OF REQUIRED IMPLEMENTATION 
  }
  ...
  for(int i=nb_layers-2; i>0; i--) {
    for(int j=0; j<layers[i].size; j++) {
	  ...
	  for (int k = 0; k < layers[i+1].size; k++) {
		  sum_weight += layers[i+1].dE_du[k] * layers[i+1].w[j][k];
	  }

	  // compute the the partial derivative of the error with respect to the activation at the current layer (layers[i].dE_du[j]) using the derivative of the activation function (deriv_transfer_f(layers[].u[]))
	  layers[i].dE_du[j] = sum_weight * deriv_transfer_f(layers[i].u[j]);
  ...
  for(int i=1; i <nb_layers ; i++) {
    for(int j=0; j<layers[i-1].size; j++) {
      ...
      int k;
      if(i!=outlayer()) k=1; //outlayer doesn't have a bias neuron but the others do
      else k=0;
	...
	//
		  layers[i].w[j][k] -= learning_rate * layers[i].dE_du[k] * layers[i-1].o[j];
...
void visualise_results(char* filename)
{
  ...
  for(int pat=0; pat<nb_pattern; pat++) {
    ...
    if(layers[outlayer()].o[0]>=.5 && layers[outlayer()].o[1]<.5 && layers[outlayer()].o[2]<.5) 
      img->put((unsigned int)pattern[pat][0],127-(unsigned int)pattern[pat][1],255,0,0);
    else if(layers[outlayer()].o[0]<.5 && layers[outlayer()].o[1]>=.5 && layers[outlayer()].o[2]<.5) 
...
int main(int argc, char ** argv)
{
  ...
  if(mode==TRAINING) {
    ...
    // ifstream InFile(argv[argc-1],std::ios::nocreate,filebuf::openprot);
    ...
    do {
      ...
      rms_error = (double)sqrt(rms_error)/(double)(nb_pattern+layers[outlayer()].size);
	...
	cout << " " << layers[0].o[i];
      cout << " NN Out=";
      for(int i=0; i<layers[outlayer()].size; i++) {
	cout << " " << layers[outlayer()].o[i] << "[" << pattern[pat][i+(layers[0].size-1)] << "]";
	if(layers[outlayer()].o[i]>=.5 && pattern[pat][i+(layers[0].size-1)]==0) {
	  classification_error++;
	  cout << "!";
	}
	else if(layers[outlayer()].o[i]<.5 && pattern[pat][i+(layers[0].size-1)]==1) {
	  classification_error++;
	  cout << "! ";
	}
      }
    ...
    cout << "Classification Errors (by output)=" << 100*classification_error/(double)(nb_pattern*layers[outlayer()].size) << "%" << endl;
